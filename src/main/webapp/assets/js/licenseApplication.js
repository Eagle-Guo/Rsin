    var COUNTRY_COUNT=0;
	function openLicenseFunction(x,y) {
          // Get the checkbox
    ...
    function choosedsv(x) {
        ...
        if (!element.classList.contains('choosesv')) {
        	switch (x) {
            case 0:
                if (services.some(el => el.name === "商标分类编号")){
                    services = services.filter(item => item.name !== "商标分类编号")
                }
                services.push({name:"商标分类编号",price:1200});
                refreshRightSummary();
                break;
            case 1:
                if (services.some(el => el.name === "需要附加大类")){
                    services = services.filter(item => item.name !== "需要附加大类")
                }
                services.push({name:"需要附加大类",price:900});
                refreshRightSummary();
                break;
            }
        } else {
