...
import sg.com.rsin.quickbooks.helper.InvoiceHelper;
import sg.com.rsin.quickbooks.helper.QBOServiceHelper;
...
public class QBOController {
	 ...
	 */
	@ResponseBody
	@GetMapping("/quickbooks/companies")
	public String callQBOCompanyInfo(HttpSession session) {
		...
		try {
			...
			// get all companyinfo
			String sql = "select * from companyinfo";//"select * from Invoice"; //
	...
	@ResponseBody
	@GetMapping("/quickbooks/invoices")
	public String callQBOInvoiceInfo(HttpSession session) {
		...
		try {
			...
			QueryResult queryResult = service.executeQuery(sql);
			return processInvoiceResponse(failureMsg, queryResult);
	...
	private String processInvoiceResponse(String failureMsg, QueryResult queryResult) {
		if (!queryResult.getEntities().isEmpty() && queryResult.getEntities().size() > 0) {
			Invoice invoice = (Invoice) queryResult.getEntities().get(0);
			logger.info("Invoiceinfo -> InvoiceDoc Number: " + invoice.getDocNumber());
			ObjectMapper mapper = new ObjectMapper();
			try {
